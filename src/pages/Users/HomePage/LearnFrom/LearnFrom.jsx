import './LearnFrom.scss';

function LearnFrom() {
  return (
    <>
      <h1 className="text-4xl font-bold text-center mb-8">Learn from Creative Experts</h1>
      <p className="flex justify-center">
        Skillshare classes are taught by industry leaders excited to share their tools, techniques, and professional journeys with you.
      </p>
      <div className="grid grid-cols-2 gap-12 w-3/6 left-1/2 relative -translate-x-1/2 mt-12">
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
        <div className="LearnFrom__img-box">
          <p className="LearnFrom__img-box-text">asdsadas</p>
          <img src="https://static.skillshare.com/assets/images/loh/marques_brownlee.webp" alt="" />
        </div>
      </div>
      <div className="grid grid-cols-2"></div>
    </>
  )
}

export default LearnFrom